    ...
    for (_, antennas) in antennas {
        ...
        while let Some((row1, col1)) = antennas.get(a) {
            ...
            while let Some((row2, col2)) = antennas.get(b) {
                ...
                let mut coords = (*row1, *col1);

                res.insert(coords);

                while let (true, new_coords) = delta_within_bounds(coords, delta, max) {
                    res.insert(new_coords);
                    coords = new_coords;
                }

                let mut coords = (*row1, *col1);
                while let (true, new_coords) =
                    delta_within_bounds(coords, (-delta.0, -delta.1), max)
                {
                    res.insert(new_coords);
                    coords = new_coords;
                }
